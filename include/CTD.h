...
public:
	...
	using End = std::array<uint16_t, 16>;

	template <class Head, class Datas, class End>
	CTD(Head&& header, Datas&& data, End&& end) : m_header(std::forward<decltype(header)>(header)), m_data(std::forward<decltype(data)>(data)), m_end(std::forward<decltype(end)>(m_end)) {}
